...
def evaluate(text_model, input_file, text_trunc_length):
    ...
    with open(osp.join(input_file), encoding='utf8') as f:
        ...
        for n, line in enumerate(data):
            out_tmp = line['output'][6:] if line['output'].startswith('[CLS] ') else line['output']
            outputs.append((line['ground_truth'], out_tmp))
...
if __name__ == "__main__":
    ...
    input_file = osp.join('prediction', model_name, 'molecular_description_generation.jsonl')
    print(f'Processing {input_file} for final metrics......')

    evaluate(args.text_model, input_file, args.text_trunc_length)
