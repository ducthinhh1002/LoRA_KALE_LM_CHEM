...
class LlaSMolGeneration(object):
    def __init__(self, model_name, base_model=None, device=None, is_llama=False):
        self.prompter = GeneralPrompter(get_chat_content, response_split="### Response:") # Modified for user-defined prompt template

        self.tokenizer, self.model = load_tokenizer_and_model(model_name, base_model=base_model, device=device, is_llama=is_llama)
        self.device = self.model.device  # TODO: check if this can work

    def create_sample(self, text, task=None, canonicalize_smiles=True, max_input_tokens=None, max_new_tokens=None):
        ...
        full_prompt = self.prompter.generate_prompt(chat, task=task, max_new_tokens=max_new_tokens)
    ...
    def generate(self, input_text, task=None, batch_size=1, max_input_tokens=512, max_new_tokens=1024, canonicalize_smiles=True, print_out=False, **generation_settings):
        ...
        for text in input_text:
            sample = self.create_sample(text, task=task, canonicalize_smiles=canonicalize_smiles, max_input_tokens=max_input_tokens, max_new_tokens=max_new_tokens)
