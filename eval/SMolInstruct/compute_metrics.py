...
def read_result(prediction_dir, task, replace_semicolon=False, read_gold_from_dataset=False, load_data_cache_dir=None):
    input_to_gold = None
    if read_gold_from_dataset:
        split_set = load_dataset('SMolInstruct/', tasks=(task,), split='test', cache_dir=load_data_cache_dir)
        input_to_gold = dict()
...
def main(prediction_dir, tasks=TASKS, load_data_cache_dir=None):
    print(f'load_data_cache_dir = {load_data_cache_dir}')
    for task in tasks:
        print('===== %s =====' % task)
